    ...
    public sealed class CurveEditor
    {
        ...
        void OnCurveGUI(Rect rect, SerializedProperty curve, CurveState state)
        {
            ...
            for (int k = 0; k < length; k++)
            {
                ...
                {
                    // Keyframe move
                    if (m_EditMode == EditMode.Moving && e.type == MouseDrag && isCurrentlySelectedCurve && isCurrentlySelectedKeyframe)
                    {
                        EditMoveKeyframe(animCurve, keys, k);
                    }

                    // Tangent editing
                    if (m_EditMode == EditMode.TangentEdit && e.type == MouseDrag && isCurrentlySelectedCurve && isCurrentlySelectedKeyframe)
                    {
                    ...
                    if (e.type == MouseDown && rect.Contains(e.mousePosition))
                    {
                    ...
                    if (e.rawType == MouseUp && m_EditMode != EditMode.None)
                    {
        ...
        void OnGeneralUI(Rect rect)
        {
            ...
            if (e.type == KeyDown && (e.keyCode == KeyCode.Delete || e.keyCode == KeyCode.Backspace))
            {
